	...
	character :: comp_file*40,image_file*40,output_file*40,parameters*60,par*60,empty*40,str*30
	...
	integer :: skip_mode,skip_number,skip_counter
	...
	par='ti-selector-large-patterns.par'
	empty=''
	...
	if (parameters.eq.empty) then
		...
		if(.not.testfl) then
			...
			write(500,*) ''
			write(500,300) 
300			format('START OF PARAMETERS:')
			write(500,301) 
301			format('samples.out                                       - samples datafile name')
			write(500,302) 
302			format('1091                                              - sample number')
			write(500,303) 
303			format('100 0.5 1                                         - nx, xmn, xsiz; in sample migration grid')
			write(500,304) 
304			format('100 0.5 1                                         - ny, ymn, ysiz; in sample migration grid')
			write(500,305) 
305			format('1   0.5 1                                         - nz, zmn, zsiz; in sample migration grid')
			write(500,306) 
306			format('ti-set.out                                        - training images datafile name')
			write(500,307) 
307			format('200 0.5 1                                         - nx, xmn, xsiz; in training images')
			write(500,308) 
308			format('200 0.5 1                                         - ny, ymn, ysiz; in training images')
			write(500,309) 
309			format('1   0.5 1                                         - nz, zmn, zsiz; in training images')
			write(500,310) 
310			format('25 25 0                                           - search radii in x,y and z directions')
			write(500,311) 
311			format('15                                                - minimum event order to analyze')
			write(500,312) 
312			format('25                                                - maximum event order to analyze')
			write(500,321)
321			format('0                                                 - skip nodes during search: 0 if no, 1 if yes')
			write(500,322)
322			format('2                                                 - keep 1 out of N nodes')
			write(500,313) 
313			format('0                                                 - compatibility mode: 0 for relative, 1 for absolute')
			write(500,314) 
314			format('1                                                 - column (for absolute mode)')
			write(500,315) 
315			format('0                                                 - tolerance parameter')
			write(500,316) 
316			format('0                                                 - variable type')
			write(500,317) 
317			format('0.1                                               - acceptance threshold (continuous variables)')
			write(500,318) 
318			format('0                                                 - training image fraction to check')
			write(500,319) 
319			format('12345                                             - random seed')
			write(500,320) 
320			format('output.out                                        - output file name')
			close(500)
		stop
		end if
	else
...
11		read(20,'(a30)',end=98) str(1:30)
		str=adjustl(str)
        	if(str(1:3).ne.'STA') go to 11
		...
		read(20,*,err=98) skip_mode
		read(20,*,err=98) skip_number
		read(20,*,err=98) mode
...
12		read(10,'(a30)',end=98) str(1:30)
		str=adjustl(str)
        	if(str(1:3).ne.'STA') go to 12
		...
		read(10,*,err=98) skip_mode
		read(10,*,err=98) skip_number
		read(10,*,err=98) mode
	...
	if (skip_mode.ne.1.and.skip_mode.ne.0) then
		goto 94
	end if
	
	if (skip_number<2) then
		goto 93
	end if
					...
					if (mcomp_grid(c29,c28,c27)==-999) then
						...
						do c30=1,nt
							...
							if (tmp_ord(c30,4)==1) then
								...
								if (skip_counter.ne.skip_number) then
									goto 501
								end if
								cond_node=cond_node+1
								c_event(cond_node,1)=offx
...
501							continue
							if (skip_counter>skip_number) then
								skip_counter=0
							end if
						end do
...
503							continue
	...
	stop
93	stop 'ERROR 1 out of N nodes will be kept, therefore the following condition needs to be met: N>1'
94	stop 'ERROR skip nodes mode has to be set to 0 or 1'
